...
상속
 ...
 - 상속 원리 :    
     super 클래스의 모든 변수와 메서드를 가져오는 것은 아니다!
     단지, super 클래스에 대한 정보를 갖고 있을 뿐이다.
...
public class Truck extends Car{
  ...
  public Truck () {
    ...
    /* !!Error!!
    autoDump = true; // 생성자 호출 후에 와야 한다.
    super();
    */
    
    // 생성자를 정의할 때 super 클래스의 생성자를 호출하는 문장을 작성하지 않으면,
    // 다음과 같이 수퍼 클래스의 "기본 생성자를 호출하는 문장이 자동으로 추가"된다.
    // => 그래서 서브 클래스의 생성자가 실행되기 전에 반드시 수퍼 틀래스의
    //    생성자가 먼저 실행된다.
