...
// 참고 : inner class는 특정 클래스의 멤버이기 때문에 
...
public class Test01 {
  ...
  static void x() {
    i = 100; // ok
    //j = 100; // nop! error => static 메서드나 블록은 this라는 내장 변수가 없다.
  }
  
  void y() {
    i = 100; // ok => 인스턴스 메서드나 블록은 클래스 변수를 바로 접근할 수 있다.
    j = 100; // error => 인스턴스 메서드나 블록은 this라는 내장 변수가 있다.
  }
  
  // 1) top level inner class
  //    static으로 선언된 inner class
  //    static이기 때문에 다른 클래스에서도 사용할 수 있다. 
  //    => 작은 크기의 클래스들을 묶고 싶을 때 top level inner class를 사용한다.
  ...
  static class TopLevelInnerClass {
    ...
    // static 영역에서는 오류지 바깥 클래스의 static 멤버(변수, 메서드)만
    // 접근할 수 있다. 
    public void test(){
  ...
  class MemberInnerClass {
    // member inner class는 내부적으로 바깥 클래스의 인스턴스 주소를 갖고 있다. 
    // 바깥 클래스의 인스턴스 주소 => 바깥클래스명.this
    public void test() {
      i = 200; // 당연히 non-static 블록에서는 static 멤버 사용 가능.
      
      // member inner class에서는 
