...
=> ServletRequest(request), HttpSession(session), 
   ServletContext(application), PageContext(page)로부터
   객체 꺼내기 또는 객체 생성
...
java.util.List products = 
  (java.util.List)request.getAttribute("products"); //객체를 꺼낼 때 쓸 이름products
if (products == null) {
...
<h2> 1. 빈 객체 생성 </h2>
<jsp:useBean 
  type="java63.servlets.test05.domain.Product" <%-- 인터페이스 적어도되 --%>
  id="product"
  class="java63.servlets.test05.domain.Product" <%-- 인터페이스 적음안되 --%>
  scope="page"/>
  
<%--
자바코드 --> 저장 되어있는지 검사 후..;; 클래스는 인스턴스 정보를 ;;
Product product = (Product)pageContext.getAttribute("product");
if(product == null) {
  product = new Product();
  }
 --%>  
<%-- product = null // null pointer exception error 출력--%>  
...
temp.setMakerNo(2);
pageContext.setAttrubute("product4", temp);
%>
<jsp:useBean 
  type="java63.servlets.test05.domain.Product" 
  id="product4"
