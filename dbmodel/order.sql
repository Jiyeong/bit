...
CREATE TABLE ADDRS (
	...
	POSTNU   VARCHAR(10)  NOT NULL COMMENT '우편번호', -- 우편번호
...
-- 임시 테이블
ALTER TABLE ADDRS
	ADD CONSTRAINT PK_ADDRS -- 임시 테이블 기본키
		PRIMARY KEY (
			ANO -- 주소번호
		);
...
-- 주문정보
CREATE TABLE ORDERS (
	ONO   INTEGER     NOT NULL COMMENT '주문정보', -- 주문정보
...
ALTER TABLE ORDERS
	MODIFY COLUMN ONO INTEGER NOT NULL AUTO_INCREMENT COMMENT '주문정보';
...
-- 주문자정보
ALTER TABLE MEMBERS
	ADD CONSTRAINT FK_ADDRS_TO_MEMBERS -- 임시 테이블 -> 주문자정보
		FOREIGN KEY (
			ANO -- 주소번호
		)
		REFERENCES ADDRS ( -- 임시 테이블
			ANO -- 주소번호
		);
